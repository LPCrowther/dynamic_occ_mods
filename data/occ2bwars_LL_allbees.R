...
##reinstated for comparison model
...
simdata <- simdata[c("year","site","time_period","L","focal")]
names(simdata) <- c("Year","Site","Visit","L","focal")
...
write.csv(out$BUGSoutput$summary, file = paste("B_hypn_no_vis_column_SS_LL_Site", format(Sys.time(), "%Y-%m-%d %I-%p"), "csv", sep = "."))
...
plot(post_50_detect  ~ surveyyear, data = out$summary, xlab = "Year", ylab = "Probability", col = "red", type = "line", ylim =c(0,1), main = "Detection = Red, Occupancy = Green, 'LL'(all bees), dashed = 95% ci")
